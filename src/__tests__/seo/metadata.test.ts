...
interface ExpectedOpenGraph {
  title: string; description: string; url: string; siteName: string; locale: string; type: string;
  images: { url: string; width: number; height: number; alt: string }[];
}
interface ExpectedTwitter { card: string; title: string; description: string; images: string[] }
interface ExpectedRobots { index: boolean; follow: boolean; googleBot: Record<string, string | number | boolean> }
...
describe("generatePageMetadata", () => {
  ...
  it("uses DEFAULT_OG_IMAGE when no image is provided", () => {
    const meta = generatePageMetadata("About Us", "Our story", "/about");
    const ogImages = (meta.openGraph as ExpectedOpenGraph)?.images;
  ...
  it("sets OpenGraph fields correctly", () => {
    ...
    const og = meta.openGraph as ExpectedOpenGraph;
  ...
  it("sets twitter card to summary_large_image with correct fields", () => {
    const meta = generatePageMetadata("Contact", "Get in touch", "/contact");
    const twitter = meta.twitter as ExpectedTwitter;
...
describe("generateLocationMetadata", () => {
  ...
  it("uses a focused local description with company and place", () => {
    const meta = generateLocationMetadata(location);
...
describe("generateBlogIndexMetadata", () => {
  ...
  it("includes a description with practical turf care tips", () => {
    const meta = generateBlogIndexMetadata();
...
describe("DEFAULT_METADATA", () => {
  ...
  it("includes the Palm Desert service area in keywords", () => {
    expect(DEFAULT_METADATA.keywords).toContain("turf cleaning Palm Desert");
  });
  ...
  it("configures OpenGraph with company details", () => {
    const og = DEFAULT_METADATA.openGraph as ExpectedOpenGraph;
  ...
  it("configures twitter card with company details", () => {
    const twitter = DEFAULT_METADATA.twitter as ExpectedTwitter;
    expect(twitter.card).toBe("summary_large_image");
  ...
  it("enables robots indexing and following", () => {
    const robots = DEFAULT_METADATA.robots as ExpectedRobots;
    expect(robots.index).toBe(true);
    expect(robots.follow).toBe(true);
  });

  it("configures googleBot with correct directives", () => {
    const googleBot = (DEFAULT_METADATA.robots as ExpectedRobots).googleBot;
